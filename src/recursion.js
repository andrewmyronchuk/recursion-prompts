...
let alternateSign = function(array) {
  ...
  var firstToAdd = array[0];
  var secondToAdd = array[1];

  if (firstToAdd < 0) {
    firstToAdd = -firstToAdd;
  }
  ...
  return [firstToAdd, secondToAdd].concat(alternateSign(array.slice(2)));
};
...
let mergeSort = function(array) {
  var merge = function(sortedArr1, sortedArr2) {
    var merged = [];
    var i = 0;
    var j = 0;

    while (i < sortedArr1.length || j < sortedArr2.length) {
      if (sortedArr1[i] <= sortedArr2[j] || sortedArr2[j] === undefined) {
...
let clone = function(input) {
  ...
  // In case input is a primitive value:
  return input;
};
